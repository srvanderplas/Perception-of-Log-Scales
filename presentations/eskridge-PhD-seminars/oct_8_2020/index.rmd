...
???
Typically, one of the first steps of a statisical analysis involves exploratory data analysis. I know at the consulting desk, the first thing I do when I get data is to generate some raw data plots. This usually guides discussions with the clients about what might be going on. However, how do we know if what we are seeing is actually there?
...
One way of answering this question is to embed the true data plot (called target plot) into what is called a lineup of randomly permuted datasets (called null plots). 

Fun fact: the term lineup comes from is named after the law-enforcement procedure to line up a suspect among a set of innocents to check if a victim can identify the suspect as the perpetrator of the crime.
...
???
So what we have is a visual test statistic which is a function that maps a dataset to a plot. We have our T(y) which maps the actual data to the desired plot and T(y_not) which maps a dataset generated under the null distribution (for example: by a randomization permutation) into that same plot.
...
Now I am going to share a few different types of lineups using some fun pictures of dogs. I know we have all become comfortable sitting on zoom on mute, but this part will require some participation where you will have to unmute yourself.
...
Actually, this plot does not contain a target panel. We call this a null lineup where all of the plots are drawn from the same null distribution. For this lineup example, the dogs aren't all the same, but it consists of 20 komondor pictures. 

A null lineup is called a Rorschach lineup which fun fact is named after the ink-blot test used in psychoanalysis. 
...
This is called a two-target lineup. The goal is to determine which target is more visually salient. Here, we have dogs for the null plots, and a fox and a cat for the two different "models". The cat is quite a bit more salient than the fox.
...
???
For those of you who were able to participate in the study. What feedback do you have?
...
???
Comparing the effect of curvature within the levels of variability:

1. We can see that within low variability it is pretty easy to pick out a target plot with Easy curvature (lots of curvature) embeded in null plots of medium curvature on both the log and linear scale, but becomes more difficult within higher variability. Notice how within that higher variability, displaying the data on the log scale helps with the accruacy. 

2. Within both levels of variability, it is easy to pick out a plot with easy curvature (a lot of curvature) within null plots of hard variability (close to a straight line) on both the log and linear scale. (i.e. easy to say something is not linear)

3. Within low variability, it is is still pretty easy to pick out a target plot with medium curvature embeded in null plots of (easy curvature) lots of curvature. It is more difficult to differentiate this within the high variability and we are maybe seeing a benefit here of displaying the data on the log scale.

4. Within low variability, it is difficult to select a target panel with medium curvature embeded in null plots with hard curvature (close to a straight line) within null panels with ha
...
???
Comparing the effect of variability within the levels of curvature, we can see it is actually very difficult to pick out the target plot. The only success we are seeing is within the easy curvature levels. This is a benefit of running pilot studies since we can narrow in on the parameter combinations that are important to include in our final study.
