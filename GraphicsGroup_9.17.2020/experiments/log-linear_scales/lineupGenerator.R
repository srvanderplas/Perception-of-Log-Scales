...
factorialData <- expand_grid(target = seq(1,6,1),
                  ...
                  select(block, trt.num, scale, panel, .n, diff.num, curvature, variability, sigma, xMid) %>%
                  arrange(block, trt.num, scale) %>%
                  # mutate(data = map2(beta,sigma,expSim)) %>%
                  mutate(data = map2(xMid,sigma,expSim)) %>%
                  unnest(data)

# Uncomment save lines but be careful because it will write over current ones.

source("save_lineups.R")
pictureDetails <- matrix(NA, nrow = nrow(ibdData), ncol = 7) %>% as.data.frame()
...
for(i in 1:nrow(ibdData)){
  ...
  # write.csv(lineupData, file = paste("data/lineupData_", i, ".csv", sep = ""), row.names = F)

  lineupPlot <- ggplot(lineupData, aes(x=x, y=y)) + 
  ...
  if(scaleID == "log"){
    lineupPlot <- lineupPlot + scale_y_continuous(trans = "log10")
  }

  # save_lineup(lineupPlot, file = paste("ineupPlot_", i, sep = ""), path = "images")
...
pictureDetails[i, "pic.name"]   <- paste("images/svg/linearLineup_", i, ".svg", sep = "")
pictureDetails[i, "target.pos"] <- pos
}

pictureDetails <- pictureDetails %>%
                    left_join(trtData, by = c("scale", "trt.num"))
