...
# With theta
expSim <- function(beta, sigma, N = n, xRange = c(0,20), yRange = c(10,100)){
...
# Create dataset
true <- expSim(beta = beta.target, sigma = sigma.target)

all.sim <- data.frame(x=NULL, y=NULL, .n=NULL)
for (i in 1:19) {
  simDat <- expSim(beta = beta.null, sigma = sigma.null)
...
# With theta
expSim <- function(xMid, sigma, N = n, xRange = c(0,20), yRange = c(10,100)){
