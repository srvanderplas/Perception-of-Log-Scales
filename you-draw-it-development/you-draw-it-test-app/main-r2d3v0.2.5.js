...
function start_drawer(state, reset = true){
  ...
  //console.log(state.drawable_points[1].x)
...
function draw_true_line({svg, line_data, draw_start}, scales){
  ...
  .attr("class", "shown_line")
  .attr("d", scales.line_drawer(df))
  .attr("fill", "none")
  .attr("stroke", "steelblue") 
  .attr("stroke-width", 4)
  // .attr(default_line_attrs);
}
...
function draw_rectangle({svg, drawable_points, draw_start, width, height, free_draw, x_by}, scales){
  ...
  const draw_region = state.svg.append("rect");
...
function draw_user_line(state, scales){
  ...
  const user_line = state.svg.append("path")
  ...
  user_line
      ...
      .attr("d", scales.line_drawer(drawable_points))
      .attr("fill", "none")
...
function draw_finished_line({svg, line_data, draw_start, free_draw}, scales){
  ...
  const finished_line = state.svg.append("path.finished_line")
...
function setup_scales(state){
  ...
  state.svg.append("g.x_grid")
  .attr('class', 'x axis-grid')
  //.translate([0,h])
  .attr("transform", `translate(0px,${h}px)`)
  .call(xAxisGrid);
  
  state.svg.append("g.y_grid")
