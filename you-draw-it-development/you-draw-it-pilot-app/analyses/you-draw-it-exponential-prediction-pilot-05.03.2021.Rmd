...
exp_simulated_data %>%
  ...
  arrange(participantID, x) %>%
  ggplot(aes(x = x)) +
  geom_line(data = exp_feedback.smooth, aes(y = ydrawn, group = interaction(participantID, scale), color = scale), alpha = 0.7) +
...
exp_simulated_data %>%
  ...
  arrange(participantID, x) %>%
  ggplot(aes(x = x)) +
  geom_line(data = exp_feedback.smooth, aes(y = yloess, group = interaction(participantID, scale), color = scale), alpha = 0.7) +
...
exp.spaghetti.plot.raw <- exp_feedback.smooth %>%
  ...
  ggplot(aes(x = x, y = residual.drawn, group = interaction(participantID, scale), color = scale)) +
  geom_line(alpha = 0.7) +
...
exp.spaghetti.plot.smooth <- exp_feedback.smooth %>%
  arrange(participantID, scale, x) %>%
  ggplot(aes(x = x, y = residual.loess, group = interaction(participantID, scale), color = scale)) +
  geom_line(alpha = 0.7) +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed", alpha = 0.5) +
  facet_grid(beta ~ points_end, scales = "free", labeller = labeller(beta = label_both, points_end = label_both)) +
  theme_bw() +
