...
<!-- \svp{May want to ask Heather about this one during the meeting (or by email)?} -->
At the beginning of the SARS-NCOV-2 pandemic, we saw an influx of dashboards being developed to display case counts, transmission rates, and outbreak regions \citep{lisa_charlotte_2020}; mass media routinely showed charts to provide information about COVID-19 \er{\citep{romano_scale_2020}}.
People began seeking out graphical displays of coronavirus data as a direct result of these pieces of work \citep{rost_2020}; providing increased and ongoing exposure to these graphics over time.
Many of these graphics helped guide decision makers to implement policies such as shut-downs or mandated mask wearing, but also facilitated communication with the public to increase compliance \er{\citep{bavel_using_2020}}.
...
We have recently experienced the benefits and pitfalls of using log-scales as covid-19 dashboards displayed case count data on both the log and linear scale \er{\citep{wade_fagen_ulmschneider_2020, financial_times_2020}}. 
...
Research suggests our perception and mapping of numbers to a number line is logarithmic at first, but transitions to a linear scale later in development, with formal mathematics education \er{\citep{varshney_why_2013, siegler_numerical_2017, dehaeneLogLinearDistinct2008}}.
...
<<<<<<< Updated upstream
<<<<<<< Updated upstream
In fact, early studies explored the estimation and prediction of exponential growth, finding that growth is underestimated when presented both numerically and graphically but that numerical estimation is more accurate than graphical estimation for exponential curves.
One way to improve estimation of increasing exponential trends \svp{is to provide} immediate feedback \svp{to participants}\citep{mackinnon_feedback_1991}.
While prior contextual knowledge or experience with exponential growth does not improve estimation, instruction on exponential growth reduces the underestimation: \svp{participants adjust their} initial starting value but not their perception of growth rate \citep{wagenaar_misperception_1975, jones_polynomial_1977}.
=======
=======
>>>>>>> Stashed changes
In fact, early studies explored the estimation and prediction of exponential growth, finding that growth is underestimated when presented both numerically and graphically but that numerical estimation is more accurate than graphical estimation for exponential curves \er{reference}.
...
These tests may take many forms: identifying differences in graphs, reading information off of a chart accurately, using data to make correct real-world decisions, and predicting the next few observations. 
...
Instead, we are simply testing the change in perceptual sensitivity resulting from visualization choices. \er{In \cite{best_perception_2007}, the authors explored whether discrimination between curve types is possible.
They found that accuracy is higher when nonlinear trends presented (e.g. it’s hard to say something is linear, but easy to say that it isn’t) and that accuracy is higher with low additive variability.}
...
In this paper, we use statistical lineups to test our ability to differentiate between exponentially increasing curves with differing growth rates, using linear and log scales.
...
In order to guarantee the simulated data spans the same range of values, we implemented a range constraint of $y\in [10,100]$ and a domain constraint of $x\in [0,20]$ with $N = 50$ points randomly assigned throughout the domain and mapped to the y-axis using the exponential model with the selected parameters. 
...
\er{
We simulated data based on a three-parameter exponential model with multiplicative errors: \svp{with parameters defined underneath}
\begin{align}
y_i & = \alpha\cdot e^{\beta\cdot x_i + \epsilon_i} + \theta \\
\text{with } \epsilon_i & \sim N(0, \sigma^2). \nonumber
\end{align} 
}
<!-- with parameters defined underneath (though without the numeric references, use \\nonumber to get rid of the numeric references on the lines) -->
...
We selected three midpoints corresponding to difficulty levels easy (obvious curvature), medium (noticeable curvature), and hard (almost linear) along with a sensible choice of standard deviation, $\sigma$. 
...
<!-- #          ) %>% -->
<!-- #   select(difficulty, alphatilde, betahat, thetahat, sigma_vals) %>% -->
<!-- #   kable("latex", digits = 2, escape = F, booktabs = T, linesep = "", align = "c", label = "parameter-data",  -->
<!-- #         col.names = c("",   "$\\alpha$", "$\\beta$", "$\\theta$", "$\\sigma^2$"),  -->
...
Two sets of each lineup parameter combination were simulated (total of 12 test data sets) and plotted on both the linear and the log scale (total of 24 test lineup plots).
...
<!-- \er{In \cite{best_perception_2007}, the authors explored whether discrimination between curve types is possible. -->
<!-- They found that accuracy higher when nonlinear trends presented (e.g. it’s hard to say something is linear, but easy to say that it isn’t) and that accuracy is higher with low additive variability.} \svp{XXX this may not be as relevant here, can you move it up to the lit review? XXX} -->
<!-- \er{We hypothesize log scales should make it much easier to estimate the growth rate since we estimate slopes relatively accurately, resolving much of the difficulty with exponential estimation \citep{mosteller_eye_1981}.} \svp{XXX You don't have to state this explicitly XXX} -->
...
The binary response was analyzed using generalized linear mixed model following a binomial distribution with a logit link function following a row-column blocking design accounting for the variation due to participant and data set respectively, with a split plot design accounting for the same participant evaluating the same data set on both the linear scale and log scale. 
See target plot identification model details and estimates in Appendix \ref{app:glmm-model}

On both the log and linear scales, the highest accuracy occured in lineup plots where the target model and null model had large curvature differences (e.g. easy-hard and hard-easy). When comparing models that have slight curvature differences (e.g. Medium-Easy), there is a sacrifice in accuracy when displayed on the linear scale.
...
odds_ratio_plot <- slice_curvature %>%
  ...
  ggplot(aes(x = Odds_Ratio_log_linear, y = Null, color = Target, shape = Target)) + 
...
\er{Our results indicated that displaying increasing exponential data on a log scale improved the accuracy of differentiating between models with slight curvature differences, particularly when identifying an exponential curve with a lower growth rate than others. 
...
The researchers found that students tended to fit the slope of the first principal component or major axis (the line that minimizes the sum of squares of perpendicular rather than vertical distances) and that students \er{who overestimated the steepness of the slope were consistent in their overestimation throughout the entire study.}
...
A similar graphical task is used in the New York Times "You Draw It" page asking readers to test their knowledge by using their curser to estimate values of a certain topic under different political administrations or over different years \er{(CITE THIS)}.
