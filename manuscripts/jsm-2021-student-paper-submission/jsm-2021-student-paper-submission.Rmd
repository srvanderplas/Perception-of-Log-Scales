...
The midpoints and standard deviation combinations were chosen using a method described in \cite{vanderplas_clusters_2017}; additional details are available in Appendix \ref{app:parameters}.
...
4. Generate $y_i = \tilde\alpha\cdot e^{\hat\beta x_i + e_i}+\hat\theta$ where $e_i\sim N(0,\sigma^2).$
...
# Lineup Evaluation Model \label{app:glmm-model}
