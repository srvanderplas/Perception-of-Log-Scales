---
title: "Eyefitting Pilot Analysis"
...
trial %>%
  ...
  geom_line(aes(y = ypcr, color = "PCR", linetype = "PCR")) +
  geom_line(aes(y = yloess, color = "Smoothed", linetype = "Smoothed")) +
  facet_wrap(~parm_id) +
  theme_bw() +
  theme(aspect.ratio = 1) +
  scale_color_manual("Estimate", values = c("steelblue", "orange", "black")) +
  scale_linetype_manual("Estimate", values = c("solid", "solid", "dashed"))
```
...
eyefitting_model_data %>%
  ggplot(aes(x = x)) +
  geom_line(aes(y = residualols.loess, group = plotID, color = "OLS"), alpha = 0.5) +
  geom_line(aes(y = residualpcr.loess, group = plotID, color = "PCR"), alpha = 0.5) +
...
eyefitting.pcr.grid.lmer  <- ref_grid(eyefitting.pcr.lmer, at = list(x = seq(1,20,0.5)))
eyefitting.pcr.preds.lmer <- emmeans(eyefitting.pcr.grid.lmer, ~ parm_id:x, cov.reduce = FALSE) %>% 
  as_tibble()

eyefitting.preds.lmer <- eyefitting.ols.preds.lmer %>%
  full_join(eyefitting.pcr.preds.lmer, by = c("x", "parm_id"), suffix = c(".ols", ".pcr"))
...
eyefitting.preds.lmer %>%
  ...
  geom_ribbon(aes(ymin = asymp.LCL.pcr, ymax = asymp.UCL.pcr, fill = "PCR"), color = NA, alpha = 0.7) +
  geom_line(aes(y = emmean.pcr, color = "PCR")) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  facet_wrap(~parm_id) +
  theme_bw() +
  theme(aspect.ratio = 1) +
